...
class ConsoleReporter {
  ...
  void reportTest(TestResult result) {
    ...
    // Main test line
    print(
      '$status ${result.suiteName}${showTimings ? ' (${duration}ms)' : ''}',
    );
    ...
    if (showDetails) {
      ...
      _reportPhaseResults(
        ...
        indent,
      ); // Note: APIs currently stored in cleanup
  ...
  void reportGroup(TestGroup group, List<TestResult> results) {
    ...
    print(
      '   ❌ Failed: ${_colorize('$failed', failed > 0 ? _ConsoleColor.red : _ConsoleColor.gray)}',
    );
    print(
      '   ⏭️  Skipped: ${_colorize('$skipped', skipped > 0 ? _ConsoleColor.yellow : _ConsoleColor.gray)}',
    );
    ...
    final rateColor = successRate == 100
        ? _ConsoleColor.green
        : successRate >= 80
        ? _ConsoleColor.yellow
        : _ConsoleColor.red;
    print(
      '   📈 Success Rate: ${_colorize('${successRate.toStringAsFixed(1)}%', rateColor)}',
    );
  ...
  void _reportPhaseResults(
    ...
    if (failedSteps.isNotEmpty || showDetails) {
      ...
      for (int i = 0; i < results.length; i++) {
        ...
        final stepColor = step.success
            ? _ConsoleColor.green
            : _ConsoleColor.red;
        final timing = showTimings
            ? ' (${step.duration.inMilliseconds}ms)'
            : '';

        print(
          '$phaseIndent${_colorize(stepIcon, stepColor)} Step ${i + 1}$timing',
        );
        ...
        if (!step.success && step.error != null) {
          print(
            '$phaseIndent  ❌ ${_colorize('Error:', _ConsoleColor.red)} ${step.error}',
          );
        }
  ...
  static void printExecutionSummary(List<TestResult> allResults) {
    ...
    final avgDuration = Duration(
      milliseconds: totalDuration.inMilliseconds ~/ allResults.length,
    );

    print('Total Tests: ${allResults.length}');
    print('✅ Passed: ${reporter._colorize('$passed', _ConsoleColor.green)}');
    print(
      '❌ Failed: ${reporter._colorize('$failed', failed > 0 ? _ConsoleColor.red : _ConsoleColor.gray)}',
    );
    print(
      '⏭️  Skipped: ${reporter._colorize('$skipped', skipped > 0 ? _ConsoleColor.yellow : _ConsoleColor.gray)}',
    );
    print(
      '⏱️  Total Duration: ${totalDuration.inSeconds}.${(totalDuration.inMilliseconds % 1000).toString().padLeft(3, '0')}s',
    );
    ...
    final rateColor = successRate == 100
        ...
        : _ConsoleColor.red;
    print(
      '📈 Success Rate: ${reporter._colorize('${successRate.toStringAsFixed(1)}%', rateColor)}',
    );

    // Show failed tests if any
    if (failed > 0) {
      print('\n❌ Failed Tests:');
      final failedResults = allResults.where(
